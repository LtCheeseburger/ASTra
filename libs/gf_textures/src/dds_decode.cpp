...
std::optional<DdsInfo> parse_dds_info(std::span<const std::uint8_t> inBytes) {
  ...
  DdsInfo info{};
  info.height = rd_u32le(hdr + 12);
  info.width  = rd_u32le(hdr + 16);
  if (info.width == 0 || info.height == 0) return std::nullopt;

  info.mipCount = rd_u32le(hdr + 28);
  ...
  std::uint32_t fourcc = fourccRaw;
  if ((pfFlags & 0x00000004u) != 0u) {
    info.format = classify_fourcc(fourcc);
    if (info.format == DdsFormat::Unknown) {
      const std::uint32_t fb = find_fourcc_fallback(hdr, 128);
      if (fb != 0) info.format = classify_fourcc(fb);
    }
  } else if ((pfFlags & 0x00000040u) != 0u) {
    // RGB. Detect common 32-bit layouts.
    if (rgbBitCount == 32 && rmask != 0 && gmask != 0 && bmask != 0) {
      (void)amask;
      info.format = DdsFormat::RGBA32;
    }
  }
