...
void MainWindow::onRsfPreviewTransformEdited(int materialIndex, const gf::models::rsf::preview_transform& transform, bool interactive) {
  ...
  refreshRsfParamsTable(materialIndex);
  if (m_rsfPreviewWidget) m_rsfPreviewWidget->updateObjectTransform(materialIndex, transform);
  setRsfDirty(true);
}
...
textTab->addAction(m_textSaveShortcutAction);
  ...
  m_rsfPreviewWidget = new gf::gui::rsf_editor::RsfPreviewWidget(m_rsfTab);

  auto* rsfSplit = new QSplitter(Qt::Horizontal, m_rsfTab);
  rsfSplit->addWidget(rsfLeft);
  rsfSplit->addWidget(m_rsfPreviewWidget);
  rsfSplit->setStretchFactor(0, 2);
  rsfSplit->setStretchFactor(1, 3);

  rsfLayout->addWidget(rsfTop, 0);
  rsfLayout->addWidget(rsfSplit, 1);
