...
GameSelectorWindow::GameSelectorWindow(QWidget* parent)
  ...
  setWindowTitle(QString("ASTra - Game Library"));
...
void GameSelectorWindow::buildUi() {
  ...
  auto* about = new QAction("About ASTra", this);
  ...
  connect(m_list, &QListWidget::customContextMenuRequested, this, [this](const QPoint& pos) {
    ...
    if (chosen == actOpen) {
      ...
      if (ok && !t.isEmpty()) {
        ...
        if (!err.isEmpty()) QMessageBox::warning(this, "ASTra", err);
        refresh();
      }
      ...
      if (ok && !u.isEmpty()) {
        const QUrl url(u);
        if (!url.isValid()) {
          QMessageBox::warning(this, "ASTra", "Invalid URL.");
        } else {
  ...
  // Module sidebar removed in v0.7.0 RC: ASTra now opens directly into the
...
const qint64 scanDurationMs = timer.elapsed();
  ...
  if (!quiet) {
    if (!err.isEmpty()) {
      QMessageBox::warning(this, "ASTra", err);
    } else {
      if (hasAst) {
        QMessageBox::information(this, "ASTra", QString("Rescan complete. Found %1 AST/BGFA file(s).").arg(astCount));
      } else {
        QMessageBox::information(this, "ASTra",
...
void GameSelectorWindow::exportDiagnosticsBundle() {
  ...
  QString readme;
  readme += "ASTra Diagnostics Bundle\n";
  readme += "------------------------\n";
...
void GameSelectorWindow::onAbout() {
  ...
  box.setWindowTitle("About ASTra");
  const QString ver = QString::fromUtf8(gf::core::kVersionString);
  QString text;
  text += QString("ASTra %1\n\n").arg(ver);
  text += "EA Sports Archive Editor / Toolsuite\n";
  text += "Current focus: AST browsing, RSF viewing/editing, APT viewing, and polish for release\n\n";
  text += "Build notes:\n";
  text += "• Game Library supports PS3/PS4/PSVita/Xbox360/WiiU title roots\n";
  text += "• Container detection: AST / TERF / BIG / Unknown (non-recursive)\n";
  text += "• AST Editor: hex viewer + text preview/editing + texture preview\n";
...
void GameSelectorWindow::onBetaTestingGuide() {
  const QString ver = QString::fromUtf8(gf::core::kVersionString);
  QString text;
  text += QString("ASTra %1 — Beta Testing Guide\n\n").arg(ver);
  text += "What we need from testers:\n";
  text += "1) Add your game root via Add Game (one per platform).\n";
  ...
  text += "   • RSF tab loads structured data for binary RSFs, identifies XML RSFs, and supports safe RSF editing\n";
  text += "3) Test export/import flows:\n";
  text += "   • RSF exports as .rsf\n";
  text += "   • P3R exports as .dds\n";
  text += "   • Replacing P3R with DDS auto-converts back for rebuild\n";
  text += "4) If something breaks, export a Diagnostics zip:\n";
  text += "   Tools > Export Selected Game Diagnostics (zip)...\n\n";
  ...
  text += "Notes:\n";
  text += "• NCAA 12 opaque/non-BGFA AST variants are still under investigation.\n";
  text += "• Some modules remain viewer-first for beta stability.\n";

  QMessageBox box(this);
  box.setWindowTitle("Beta Testing Guide");
  box.setText(text);
...
void GameSelectorWindow::onRemoveGame() {
  ...
  if (!m_library->removeById(id, &err)) {
    QMessageBox::warning(this, "ASTra", err.isEmpty() ? "Failed to remove game." : err);
    return;
  }
  refresh();
}
