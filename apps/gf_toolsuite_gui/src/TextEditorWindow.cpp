...
void TextEditorWindow::updateUi() {
  ...
  m_doc.dirty = dirty;
  setWindowTitle(m_doc.makeWindowTitle("ASTra", "Text Editor"));
...
bool TextEditorWindow::openFile(const QString& path) {
  ...
  if (!f.open(QIODevice::ReadOnly)) {
    QMessageBox::warning(this, "ASTra", QString("Failed to open file:\n%1").arg(path));
    return false;
  }
  ...
  if (f.size() > kMaxBytes) {
    QMessageBox::warning(this, "ASTra", "File is too large for the text editor (32 MiB limit)." );
    return false;
  }
...
bool TextEditorWindow::openTextContent(const QString& title, const QString& text, const QString& suggestedFileName) {
  ...
  setWindowTitle(QString("ASTra - Text Editor - %1").arg(title));
...
bool TextEditorWindow::saveToPath(const QString& path) {
  ...
  if (!res.ok) {
    QMessageBox::warning(this, "ASTra", QString("Save failed:\n%1").arg(QString::fromStdString(res.message)));
    return false;
  }
...
void TextEditorWindow::onRevert() {
  ...
  const auto rc = QMessageBox::question(this, "ASTra", "Revert to last saved state?", QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
