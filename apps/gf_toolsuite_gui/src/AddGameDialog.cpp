...
static bool promptForGameTitle(QWidget* parent, GameEntry& g) {
  ...
  const QString hint = QString(
    "ASTra couldn't determine this game's title automatically (%1).\n\n"
    "Enter a name for the game. ASTra will use it to download the correct cover art and for display in the library.")
      .arg(plat);
...
AddGameDialog::AddGameDialog(GameLibrary* lib, QWidget* parent)
  ...
  m_hint = new QLabel(
    "Select a game folder. ASTra will auto-detect the platform:\n"
    "• PS2: SYSTEM.CNF\n"
...
void AddGameDialog::onAccept() {
  ...
  if (root.isEmpty()) {
    QMessageBox::warning(this, "ASTra", "Please select a folder.");
    return;
  }
  if (!dirExists(root)) {
    QMessageBox::warning(this, "ASTra", "That folder does not exist.");
    return;
  }
    ...
    if (d.dirName().compare("USRDIR", Qt::CaseInsensitive) == 0) {
      ...
      if (QDir(parent).dirName().compare("PS3_GAME", Qt::CaseInsensitive) == 0 &&
        ...
        if (!offerAutoFix(
              ...
              "ASTra expects the *game root* folder (the folder containing PS3_GAME).",
              grand)) {
          return;
        }
      }
    ...
    if (name.compare("USRDIR", Qt::CaseInsensitive) == 0) {
      ...
      if (QDir(parent).dirName().compare("PSP_GAME", Qt::CaseInsensitive) == 0 &&
        ...
        if (!offerAutoFix(
              ...
              "ASTra expects the *PSP root* folder (the folder containing PSP_GAME).",
              grand)) {
          return;
        }
      }
  ...
  GameEntry g;
  QString err;
  // For now ASTra only opens BGFA/AST based games, but we persist all platforms
  ...
  if (det.platformId == platform_id::kPS3) {
    if (!detectPs3Game(root, &g, &err)) {
      QMessageBox::warning(this, "ASTra", err.isEmpty() ? "Failed to detect PS3 game." : err);
      return;
    }
  } else if (det.platformId == platform_id::kPS4) {
    if (!detectPs4Game(root, &g, &err)) {
      QMessageBox::warning(this, "ASTra", err.isEmpty() ? "Failed to detect PS4 game." : err);
      return;
    }
  } else if (det.platformId == platform_id::kPSVita) {
    if (!detectPsVitaGame(root, &g, &err)) {
      QMessageBox::warning(this, "ASTra", err.isEmpty() ? "Failed to detect PSVita game." : err);
      return;
    }
  } else if (det.platformId == platform_id::kPSP) {
    if (!detectPspGame(root, &g, &err)) {
      QMessageBox::warning(this, "ASTra", err.isEmpty() ? "Failed to detect PSP game." : err);
      return;
    }
  } else if (det.platformId == platform_id::kXbox360) {
    if (!detectXbox360Game(root, &g, &err)) {
      QMessageBox::warning(this, "ASTra", err.isEmpty() ? "Failed to detect Xbox 360 game." : err);
      return;
    }
  } else {
  ...
  if (!m_lib->addOrReplace(g, &saveErr)) {
    QMessageBox::warning(this, "ASTra", saveErr.isEmpty() ? "Failed to save game library." : saveErr);
    return;
  }
